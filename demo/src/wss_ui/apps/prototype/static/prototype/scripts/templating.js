...
function fill_data_list(selector) {
    ...
    if (selector !== undefined) base = $(selector).find(".fill-data-list");
    ...
    if (tm.length > 0) {
        ...
        for (var vi in l) {
            ...
            $(f).find(".fill-data-list-item").each(function (ii, ie) {
                ...
                var vn = base_c + c + "[" + vi + "].";
                if (fm == "list") fill_data_list_el(it, base_c + c + "[" + vi + "]" + ".");else {
                    ...
                    if (ic.indexOf("|") >= 0) {
                        ...
                        for (var ici in ics) {
                            var lvn = vn + ics[ici];
                            var v = eval(lvn);
                            if (igs[ici] == "class") it.addClass(v + "");else it.attr(igs[ici], v);
                        }
                    } else {
                        vn += ic;
                        ...
                        var v = eval(vn);
                        if (ig) {
                            if (ig == "class") it.addClass(v + "");else it.attr(ig, v);
                        } else {
                            it.text(v);
                        }
                    }
...
function fill_data() {
    $(".fill-data").each(function (i, e) {
        ...
        if (g) {
            if (g == "class") t.addClass(v + "");else t.attr(g, v);
        } else {
            t.text(v);
        }
    });
